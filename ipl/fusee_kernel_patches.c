...
static const kernel_info_t g_kernel_infos[] = {
    {   /* 1.0.0. */
        .pkg1_id = "20161121183008",
        .free_code_space_offset = 0x4597C,
        KERNEL_HOOKS(100)
    },
    {   /* 2.0.0. */
        /* TODO */
        .pkg1_id = "20170210155124",
        .free_code_space_offset = 0,
        KERNEL_HOOKS(200)
    },
    {   /* 3.0.0. */
        .pkg1_id = "20170519101410",
    ...
    {   /* 3.0.2. */
        /* TODO */
        .pkg1_id = "20170710161758",
        .free_code_space_offset = 0,
        KERNEL_HOOKS(302)
    },
    {   /* 4.0.0. */
        .pkg1_id = "20170921172629",
...
const kernel_info_t *get_kernel_info(const char* pkg1_id) {
    for (unsigned int i = 0; i < sizeof(g_kernel_infos) / sizeof(kernel_info_t); i++)
    {
        if (strcmp(g_kernel_infos[i].pkg1_id, pkg1_id) == 0)
        {
...
void package2_patch_kernel(gfx_con_t* gfx_con, const char* pkg1_id, void *_kernel, size_t size) {
    const kernel_info_t *kernel_info = get_kernel_info(pkg1_id);
    if (kernel_info == NULL) {
